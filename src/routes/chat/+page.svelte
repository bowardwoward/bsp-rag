<script lang="ts">
		...
		messages = [
			{
				role: 'assistant',
				content:
					"Hello! I'm your BSP Legal Assistant. How can I help you with Philippine banking and financial regulations today?"
			}
		];
	});
	...
	async function handleSubmit() {
		...
		try {
			...
			// Make sure search results are properly parsed if they're coming from the API
			const formattedResults = searchResults.map((result) => ({
				documentId:
					typeof result.documentId === 'string' ? parseInt(result.documentId) : result.documentId,
...
<div class="flex flex-col h-full max-w-4xl mx-auto">
	<div class="mb-4 flex justify-between items-center">
		...
		<button class="px-3 py-1 bg-slate-200 hover:bg-slate-300 rounded" on:click={goHome}>
			← Back
		</button>
	</div>
			...
			<div class={`mb-4 ${message.role === 'user' ? 'text-right' : ''}`}>
				<div
					class={`inline-block p-3 rounded-lg ${
						message.role === 'user' ? 'bg-[#FFA500] text-white' : 'bg-gray-100 text-gray-800'
